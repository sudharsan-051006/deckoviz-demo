...
export const DynamicImageGrid: React.FC<DynamicImageGridProps> = ({ 
  ...
  // --- NEW --- Extracted logic into its own function for clarity
  ...
  // --- LOGIC --- useEffect is now cleaner and uses the new function
  useEffect(() => {
    // Initial display of 4 images
    setDisplayedImages(imageSources.slice(0, 4));

    // --- FIX --- Use a more generic timer type to avoid NodeJS errors
  ...
  return (
    <section className="py-16 sm:py-20 md:py-24 bg-gradient-to-br from-gray-50 to-white overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div 
          className="grid grid-cols-2 md:grid-cols-4 gap-5 md:gap-7 relative group"
          ...
          {displayedImages.map((image, index) => {
            ...
            return (
              <div 
                ...
                className="relative aspect-square bg-gray-200 rounded-3xl overflow-hidden shadow-xl transition-shadow duration-300 hover:shadow-2xl group/item"
              >
                <img
                  src={image.src}
                  alt={image.tag ? `Image for ${image.tag}` : `Dynamic grid image ${index + 1}`}
                  className={`
